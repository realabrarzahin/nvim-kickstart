...
vim.api.nvim_set_hl(0, 'cursorline', { bg = '#060709', blend = 40 })
...
end, { desc = 'Open Vertical Split (1/4 Width)' })

vim.api.nvim_set_keymap('n', '<leader>so', ':split out<CR>', { noremap = true, silent = true })
...
vim.keymap.set('n', '<leader>tt', function()
  if term_buf_nr and vim.api.nvim_buf_is_loaded(term_buf_nr) then
    local winid = vim.fn.bufwinid(term_buf_nr)
    if winid ~= -1 then
    ...
    vim.cmd 'resize 10'
    vim.cmd 'term'
    term_buf_nr = vim.api.nvim_get_current_buf()
    vim.cmd 'startinsert'
  end
